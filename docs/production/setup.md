...
新しいロゴ (紺と緑、上から「車のマーク / カタカナ / 英字」の構成。背景が透明な PNG か白地の画像) に差し替えるときは:

1. 新しいロゴを `images/brand/logo-original.png` として置く (いまの元画像を上書き)
2. 次を実行する (白地用・黒地用・横長版・アイコン・共有画像をまとめて作り直し、各ページの画像サイズの指定も合わせます)
