...
**2026-09-23 更新: 本番実装 v1 は完了しました** ([実装契約書](implementation-v1.md)・[実装状況と検証結果](current-state-audit.md#10-2026-09-23-実装状況))。
予約・会員・管理画面は Supabase (DB・認証・Edge Functions) で動き、二重予約の DB 制約、サーバーでの料金計算、二段階認証、
行レベルの権限、同意の版管理、監査ログ、メール送信キュー、Google カレンダー連携まで、ローカルの本番相当環境で検証済みです。
設定を入れないうちは従来どおりのデモとして動きます。

ただし、**実際のお客様の予約を受け付けるのは、[立ち上げ手順書](setup.md) のチェックリストと、下の事業判断・法務確認が済んでから**です。

<details><summary>2026-09-16 時点の結論 (実装前)</summary>
